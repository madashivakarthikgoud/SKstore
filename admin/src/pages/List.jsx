...
const List = ({ token }) => {
  ...
  const removeProduct = async (id) => {
    try {
      const response = await axios.delete(`${backendUrl}/api/product/remove/${id}`, { headers: { token } }); // Using DELETE method
      if (response.data.success) {
    ...
    <>
      ...
      <div className='flex flex-col gap-2'>
        ...
        {list.map((item, index) => (
          ...
          >
            <img className='w-12' src={item.images[0].url} alt={item.name} /> {/* Ensure correct image URL */}
            <p>{item.name}</p>
            <p>{item.category}</p>
            <p>
              {currency}
